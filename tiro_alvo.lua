...
local objetos= {
            {"img_jogos/BI/hemacia.png", "Hemacia", ""},
            {"img_jogos/BI/leucocito.png", "Leucocito", ""},
            {"img_jogos/BI/plaqueta.png", "Plaqueta", ""},
            {"img_jogos/BI/coccix.png", "Coccix", ""},
            {"img_jogos/BI/fibula.png", "Fibula", ""},
            {"img_jogos/BI/femur.png", "Femur", ""},
            {"img_jogos/BI/sacro.png", "Sacro", ""},
            {"img_jogos/BI/radio.png", "Radio", ""},
            {"img_jogos/BI/tibia.png", "Tibia", ""},
            {"img_jogos/BI/ulna.png", "Ulna", ""},
            {"img_jogos/BI/umero.png", "Umero", ""}
         }
...
function touchMe( event)
    if (event.phase == "began") then
        if (event.target.id == alvo) then
            ...
            if (tonumber(scorePoint.text) > tonumber(bsPoint)) then
                bsPoint = tonumber(scorePoint.text)
            end
...
function selecionaAlvo()
    print("entroue")
    alvo = math.random(4,11)

    alvo = objetos[alvo][2]

    viewTarget.text = "Target: "..alvo

    timer.performWithDelay( 15000, selecionaAlvo )
...
function tiro_alvo:show(event)
    if (event.phase == "will") then
        ...
        viewTarget = display.newText("Target: ", display.contentCenterX * 0.1, display.contentCenterY * 0.035)
        viewTarget:setFillColor(1,1,1)
        viewTarget.anchorX = 0
        SceneGroup:insert(viewTarget)
